...
let higherOrderInputSearch = input.pipe(
...
// Wir müssen die beiden Datenströme wieder zusammenführen. Dazu gibt es verschiedene Merge-Strategien.
...
// Suchergebnis erhalten hat. Man könnte die Suchergebnisse in ihrem zeitlichen Eintreffen zurückgegen (mergeMap) oder 
// in der Reihenfolge, die durch die Sucheingaben vorgegeben ist (concatMap). Beides ist in unseren Use-Case nicht sinnvoll.
// Sobald ein neuer Suchstring emitiert wird, interessiert uns das alte Suchergebnis nicht mehr. Die alte Subscription sollte
// beendet werden und das veraltete Suchergebnis erst garnicht im Ergebnis-Stream auftauchen. Das macht switchMap:

let inputSearch = input.pipe(
  switchMap(searchString => search(searchString))
);

// Mit dieser simplen Verknüpfung haben wir eine Suche implementiert, die veraltete Suchergebnisse nicht anzeigt (Race-Problem gelöst)
// und die die Möglichkeit bietet, veraltete Requests abzubrechen. Hier sieht man die Eleganz und Stärke von RxJS.
